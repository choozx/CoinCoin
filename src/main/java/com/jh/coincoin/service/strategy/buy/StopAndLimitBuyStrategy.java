...
//    }
    ...
    @Override
    public void order(Symbol symbol, Side side, int leverage, RiskRewardRatioDto riskRewardRatioDto, double orderBalanceRatio) {
        // 1.주문된 상태 체크 redis에서 주문정보 get-> 주문된 상태면 return
        long now = DateTimeUtil.getCurrentTimeMillis();
        ...
        NewOrderReq tkOrder = NewOrderReq.builder()
                ...
                .type(Order.TAKE_PROFIT_MARKET)
                .quantity(CommonUtil.formatDecimal(quantity, 3))
                .stopPrice(CommonUtil.formatDecimal(calculator.calcPrice(side, Order.TAKE_PROFIT_MARKET, entryPrice, riskRewardRatioDto.getLimit()), 2))
                .closePosition(true)
                .timestamp(now)
                .build();
        binanceFutureAPIService.newOrder(tkOrder);
        ...
        NewOrderReq slOrder = NewOrderReq.builder()
                ...
                .type(Order.STOP_MARKET)
                .quantity(CommonUtil.formatDecimal(quantity, 3))
                .stopPrice(CommonUtil.formatDecimal(calculator.calcPrice(side, Order.STOP_MARKET, entryPrice, riskRewardRatioDto.getStop()), 2))
                .closePosition(true)
                .timestamp(now)
                .build();
        binanceFutureAPIService.newOrder(slOrder);
